document.addEventListener('DOMContentLoaded', () => {
    ...
    const randomEvents = [ 
        { title: "Customer Complaint!", description: "A customer says their Cacio e Pepe is too peppery!", options: [ 
        ...
        { title: "Kitchen Emergency!", description: "The oven suddenly stopped working!", options: [ 
        ...
        { title: "Ingredient Shortage", description: "Oh no! We're running low on fresh basil for Pomodoro!", options: [ 
        ...
        { title: "VIP Guest", description: "A famous food critic just sat down at Table 3!", options: [ 
    ...
    // --- Event Listeners ---
    // Keep this for when users click "Start Day" button, but we'll also start game automatically
    if (startBtn) {
    ...
    function endGame() {
        ...
        // DO NOT make start button reappear
    ...
    function customerLeavesAngry(c) { 
        ...
        c.state = 'leaving'; 
        c.tableElement.classList.remove('table-highlight'); 
        showFeedbackIndicator(c.tableElement, "Left Angry! 😡", "negative"); 
        if (c.element) { 
    ...
    function movePlayerToElement(targetEl, callback = null) { 
        ...
        if (targetEl.closest('.kitchen-row')) { 
            ...
            const minX = plyW + 5; 
            const maxX = restaurantArea.offsetWidth - plyW - 5; 
            ...
            console.warn("Move target unknown:", targetEl); 
            return; 
        } 
    ...
    function movePlayerToCoordinates(tX, tY, callback = null) { 
        ...
        isMoving = true; 
        const sX = playerPosition.x; 
        const sY = playerPosition.y; 
    ...
    function updatePlayerPosition() { 
        ...
        const maxX = restaurantArea.offsetWidth - plyW - 5; 
        const minY = plyH + 5; 
        const maxY = restaurantArea.offsetHeight - plyH - 5; 
    ...
    function spawnCustomer() { 
        if (!gameRunning || isPaused) return; 
        const availT = Array.from(tables).filter(t => !customers.some(c => c.tableElement === t && c.state !== 'leaving' && c.state !== 'remove')); 
        if (availT.length > 0) { 
            const t = availT[Math.floor(Math.random() * availT.length)]; 
            const s = t.querySelector('.seat'); 
            if (!s) { 
            ...
            const fData = foodItems[order]; 
            const bub = document.createElement('div'); 
            bub.className = 'speech-bubble'; 
            bub.innerHTML = `<div class="dish-name">${order}</div><div class="dish-price">${fData.price}</div><div class="dish-emoji">${fData.emoji}</div>`; 
            bub.style.opacity = '0'; 
            const moodI = document.createElement('div'); 
            moodI.className = 'mood-indicator'; 
            moodI.textContent = moodEmojis.happy; 
            custEl.appendChild(moodI); 
            custEl.appendChild(bub); 
            s.appendChild(custEl); 
            requestAnimationFrame(() => { 
                custEl.style.opacity = '1'; 
                bub.style.opacity = '1'; 
            }); 
            const cId = `cust-${Date.now()}-${Math.random().toString(16).slice(2)}`; 
            const pat = Math.max(15, customerPatienceBase - (level - 1) * 2); 
            const nC = { 
                id: cId, 
                element: custEl, 
                tableElement: t, 
                order: order, 
                orderPrice: fData.price, 
                spawnTime: Date.now(), 
                patienceTotal: pat, 
                patienceCurrent: pat, 
                moodIndicator: moodI, 
            ...
            customers.push(nC); 
            t.classList.add('table-highlight'); 
        } 
        scheduleNextCustomer(); 
    }
    ...
    function serveCustomer(cust) { 
        ...
        cust.state = 'served'; 
        const t = cust.tableElement; 
        const p = cust.orderPrice; 
        const pR = Math.max(0, cust.patienceCurrent) / cust.patienceTotal; 
        let tipM = 0.05; 
        if (pR > 0.8) tipM = 0.20; 
        else if (pR > 0.5) tipM = 0.15; 
        else if (pR > 0.2) tipM = 0.10; 
        const tipA = Math.ceil(p * tipM); 
        const totE = p + tipA; 
        money += totE; 
        moneyDisplay.textContent = money; 
        showFeedbackIndicator(t, `+ ${p}<br/>+ ${tipA} tip!`, "positive"); 
        cust.moodIndicator.textContent = '😋'; 
        const bub = cust.element.querySelector('.speech-bubble'); 
        if (bub) bub.innerHTML = "Grazie! 👌"; 
        t.classList.remove('table-highlight'); 
        carryingFood = null; 
    ...
    function updateCustomerMood(cust) { 
        ...
        const pR = Math.max(0, cust.patienceCurrent) / cust.patienceTotal; 
        let m = moodEmojis.happy; 
        if (pR <= 0.2) m = moodEmojis.angry; 
        else if (pR <= 0.5) m = moodEmojis.impatient; 
        else if (pR <= 0.8) m = moodEmojis.neutral; 
        cust.moodIndicator.textContent = m; 
    }
    
    function checkLevelUp() { 
        const nextL = level; 
        if (nextL >= levelThresholds.length) return; 
        const needed = levelThresholds[nextL]; 
        if (money >= needed) { 
            level++; 
    ...
    function clearCustomersAndIndicators() { 
        ...
        tables.forEach(t => t.classList.remove('table-highlight')); 
    }
    ...
    function populateMenuModal() { 
        ...
        const cats = {}; 
        for (const iN in foodItems) { 
            const i = foodItems[iN]; 
            if (!cats[i.category]) cats[i.category] = []; 
            cats[i.category].push({ name: iN, ...i }); 
        } 
        const tOrd = Array.from(tabBtns).map(b => b.getAttribute('data-tab')); 
        tOrd.forEach(tK => { 
            let catN = getCategoryNameFromTabKey(tK); 
            let items = []; 
            if (tK === 'mains') { 
                items = [...(cats['Mains'] || []), ...(cats['Sides'] || [])]; 
                if (!items.length) catN = null; 
            } else { 
                items = cats[catN]; 
            } 
            if (items && items.length > 0) { 
                const sDiv = document.createElement('div'); 
                sDiv.className = 'menu-section'; 
                sDiv.setAttribute('data-section', tK); 
                items.forEach(it => { 
                    const iDiv = document.createElement('div'); 
                    iDiv.className = 'menu-item'; 
                    iDiv.innerHTML = `<h5>${it.name} ${it.emoji} - ${it.price}</h5><p>Prep Time: ${it.prepTime}s</p>`; 
                    sDiv.appendChild(iDiv); 
                }); 
                menuSectionsContainer.appendChild(sDiv); 
            } 
        }); 
    }
    
    function getCategoryNameFromTabKey(tK) { 
        switch(tK) { 
